...
(* let mbc1_no_ram rom_banks : (module S) = (module struct
  ...
  let initial = { rom = Bytes.empty; rom_bank = 0b00001; ram_bank = 0b00; mode = M0 }
  ...
  let get m i =
    match i with
    | _ when in_low i  -> Bytes.get m.rom (addr_low m i) |> int_of_char
    | _ when in_high i -> Bytes.get m.rom (addr_high m i) |> int_of_char
    | _                -> 0xFF
...
module RAM = (val RAM.make_chunk 512 0xA000)
  ...
  let get m i =
    ...
    | _ when m.ram_enabled ->
      RAM.get m.ram (0xA000 + (i land 0x17))
    | _ ->
      0xFF
  ...
  let set m i v =
    ...
    | _ when in_ram i ->
      { m with ram = RAM.set m.ram (0xA000 + (i land 0x17)) (v land 0b1111) }
    | _ -> m
