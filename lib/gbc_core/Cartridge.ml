...
module With_RAM : S = struct
  ...
  let get (rom, ram) i =
    if in_rom i then String.get rom i |> int_of_char else Bank.get ram i

  let set (rom, ram) i v =
    if in_rom i then (rom, ram) else (rom, Bank.set ram i v)
...
let mbc1 rom_banks ram_banks : (module S) =
  (module struct
    ...
    let get m i =
      match i with
      | _ when in_low i -> String.get m.rom (addr_low m i) |> int_of_char
      | _ when in_high i -> String.get m.rom (addr_high m i) |> int_of_char
      | _ when m.ram_enabled -> RAM.get m.ram (ram_addr m i)
      | _ -> 0xFF

    let rom_size_bitmask = function
      | 2 -> 0b00000001
      | 4 -> 0b00000011
      | 8 -> 0b00000111
      | 16 -> 0b00001111
    ...
    let set m i v =
      ...
      | _ when 0x2000 <= i && i <= 0x3FFF ->
          ...
          { m with rom_bank }
      | _ when 0x4000 <= i && i <= 0x5FFF -> { m with ram_bank = v land 0b11 }
...
let mbc2 : (module S) =
  (module struct
    ...
    let initial =
      { rom = ""; ram = RAM.initial; rom_bank = 1; ram_enabled = false }
    ...
    let get m i =
      match i with
      | _ when in_low i -> String.get m.rom i |> int_of_char
      | _ when in_high i ->
          String.get m.rom ((0x4000 * m.rom_bank) + i - 0x4000) |> int_of_char
      | _ when m.ram_enabled -> RAM.get m.ram (0xA000 + (i land 0x1F))
      | _ -> 0xFF
    ...
    let set m i v =
      ...
      | _ when in_low i ->
          if i land (0b1 lsl 8) = 0 then
            if v = 0x0A then { m with ram_enabled = true }
            else { m with ram_enabled = false }
          else
      ...
      | _ when in_ram i ->
          {
            m with
            ram = RAM.set m.ram (0xA000 + (i land 0x1F)) (v land 0b1111);
          }
      | _ -> m
