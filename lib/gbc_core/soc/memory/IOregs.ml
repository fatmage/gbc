...
module Joypad = struct
  ...
  let set m _ v = { m with chosen = v land 0x30 }

  let set_input m buttons dpad =
    match (buttons, dpad) with
    | buttons, dpad when buttons = m.buttons && dpad = m.dpad -> m
    | buttons, _ when dpad = m.dpad -> { m with buttons }
    | _, dpad when buttons = m.buttons -> { m with dpad }
    | buttons, dpad -> { m with buttons; dpad }
...
module Serial = struct
  ...
  let get m i =
    match i with 0xFF01 -> m.sb | 0xFF02 -> m.sc | _ -> assert false
...
module Timer = struct
  ...
  let inc_tima m =
    let res = m.tima + 1 in
    if res > 0xFF then ({ m with tima = m.tma }, true)
    else ({ m with tima = res }, false)
  ...
  let mcyc_to_hz v double = if double then 1048576 / v else 2097152 / v
  ...
  let run_div m cycles =
    let rec aux m = function 0 -> m | n -> aux (inc_div m) (n - 1) in
