...
module Make (State : State.S) : S = struct
  ...
  let fetch_decode (st : State.t) : Instruction.instruction * int =
    ...
    | 0x01 ->
        let n16 = State.Bus.get16 st (st.regs._PC + 1) in
        (Instruction.iLD_rn16 BC n16, 3)
    | 0x02 -> (Instruction.iLD_r16pA BC, 1)
    | 0x03 -> (Instruction.iINC_r16 BC, 1)
    | 0x04 -> (Instruction.iINC_r8 B, 1)
    | 0x05 -> (Instruction.iDEC_r8 B, 1)
    | 0x06 ->
        let n8 = State.Bus.get8 st (st.regs._PC + 1) in
        (Instruction.iLD_rn8 B n8, 2)
    | 0x07 -> (Instruction.iRLCA, 1)
    | 0x08 ->
        let n16 = State.Bus.get16 st (st.regs._PC + 1) in
        (Instruction.iLD_n16pSP n16, 3)
    ...
    | 0x0E ->
        let n8 = State.Bus.get8 st (st.regs._PC + 1) in
        (Instruction.iLD_rn8 C n8, 2)
    | 0x0F -> (Instruction.iRRCA, 1)
    | 0x10 -> Instruction.iSTOP st
    | 0x11 ->
        let n16 = State.Bus.get16 st (st.regs._PC + 1) in
        (Instruction.iLD_rn16 DE n16, 3)
    ...
    | 0x16 ->
        let n8 = State.Bus.get8 st (st.regs._PC + 1) in
        (Instruction.iLD_rn8 D n8, 2)
    | 0x17 -> (Instruction.iRLA, 1)
    | 0x18 ->
        let n8 = State.Bus.get8 st (st.regs._PC + 1) in
        (Instruction.iJR_n8 n8, 2)
    ...
    | 0x1E ->
        let n8 = State.Bus.get8 st (st.regs._PC + 1) in
        (Instruction.iLD_rn8 E n8, 2)
    | 0x1F -> (Instruction.iRRA, 1)
    | 0x20 ->
        let n8 = State.Bus.get8 st (st.regs._PC + 1) in
        (Instruction.iJR_cn8 Cnz n8, 2)
    | 0x21 ->
        let n16 = State.Bus.get16 st (st.regs._PC + 1) in
        (Instruction.iLD_rn16 HL n16, 3)
    ...
    | 0x26 ->
        let n8 = State.Bus.get8 st (st.regs._PC + 1) in
        (Instruction.iLD_rn8 H n8, 2)
    | 0x27 -> (Instruction.iDAA, 1)
    | 0x28 ->
        let n8 = State.Bus.get8 st (st.regs._PC + 1) in
        (Instruction.iJR_cn8 Cz n8, 2)
    ...
    | 0x2E ->
        let n8 = State.Bus.get8 st (st.regs._PC + 1) in
        (Instruction.iLD_rn8 L n8, 2)
    | 0x2F -> (Instruction.iCPL, 1)
    | 0x30 ->
        let n8 = State.Bus.get8 st (st.regs._PC + 1) in
        (Instruction.iJR_cn8 Cnc n8, 2)
    | 0x31 ->
        let n16 = State.Bus.get16 st (st.regs._PC + 1) in
        (Instruction.iLD_SPn16 n16, 3)
    ...
    | 0x36 ->
        let n8 = State.Bus.get8 st (st.regs._PC + 1) in
        (Instruction.iLD_HLpn8 n8, 2)
    | 0x37 -> (Instruction.iSCF, 1)
    | 0x38 ->
        let n8 = State.Bus.get8 st (st.regs._PC + 1) in
        (Instruction.iJR_cn8 Cc n8, 2)
    ...
    | 0xC2 ->
        let n16 = State.Bus.get16 st (st.regs._PC + 1) in
        (Instruction.iJP_cn16 Cnz n16, 3)
    | 0xC3 ->
        let n16 = State.Bus.get16 st (st.regs._PC + 1) in
        (Instruction.iJP_n16 n16, 3)
    | 0xC4 ->
        let n16 = State.Bus.get16 st (st.regs._PC + 1) in
        (Instruction.iCALL_cn16 Cnz n16, 3)
    | 0xC5 -> (Instruction.iPUSH_r16 BC, 1)
    | 0xC6 ->
        let n8 = State.Bus.get8 st (st.regs._PC + 1) in
        (Instruction.iADD_An8 n8, 2)
    | 0xC7 -> (Instruction.iRST_vec 0x00, 1)
    | 0xC8 -> (Instruction.iRET_c Cz, 1)
    | 0xC9 -> (Instruction.iRET, 1)
    | 0xCA ->
        let n16 = State.Bus.get16 st (st.regs._PC + 1) in
        (Instruction.iJP_cn16 Cz n16, 3)
    (* | 0xCB ->  prefix CB *)
    | 0xCC ->
        let n16 = State.Bus.get16 st (st.regs._PC + 1) in
        (Instruction.iCALL_cn16 Cz n16, 3)
    | 0xCD ->
        let n16 = State.Bus.get16 st (st.regs._PC + 1) in
        (Instruction.iCALL_n16 n16, 3)
    | 0xCE ->
        let n8 = State.Bus.get8 st (st.regs._PC + 1) in
        (Instruction.iADC_An8 n8, 2)
    | 0xCF -> (Instruction.iRST_vec 0x08, 1)
    | 0xD0 -> (Instruction.iRET_c Cnc, 1)
    | 0xD1 -> (Instruction.iPOP_r16 DE, 1)
    | 0xD2 ->
        let n16 = State.Bus.get16 st (st.regs._PC + 1) in
        (Instruction.iJP_cn16 Cnc n16, 3)
    (* | 0xD3 ->  iNOP*)
    | 0xD4 ->
        let n16 = State.Bus.get16 st (st.regs._PC + 1) in
        (Instruction.iCALL_cn16 Cnc n16, 3)
    | 0xD5 -> (Instruction.iPUSH_r16 DE, 1)
    | 0xD6 ->
        let n8 = State.Bus.get8 st (st.regs._PC + 1) in
        (Instruction.iSUB_An8 n8, 2)
    | 0xD7 -> (Instruction.iRST_vec 0x10, 1)
    | 0xD8 -> (Instruction.iRET_c Cc, 1)
    | 0xD9 -> (Instruction.iRETI, 1)
    | 0xDA ->
        let n16 = State.Bus.get16 st (st.regs._PC + 1) in
        (Instruction.iJP_cn16 Cc n16, 3)
    (* | 0xDB -> Instruction.iNOP  *)
    | 0xDC ->
        let n16 = State.Bus.get16 st (st.regs._PC + 1) in
        (Instruction.iCALL_cn16 Cc n16, 3)
    (* | 0xDD -> Instruction.iNOP  *)
    | 0xDE ->
        let n8 = State.Bus.get8 st (st.regs._PC + 1) in
        (Instruction.iSBC_An8 n8, 2)
    | 0xDF -> (Instruction.iRST_vec 0x18, 1)
    | 0xE0 ->
        let n8 = State.Bus.get8 st (st.regs._PC + 1) in
        (Instruction.iLDH_n16pA n8, 2)
  ...
  let first_set_bit =
    let rec aux n m k =
      match n land m with 0 -> aux n (m * 2) (k + 1) | _ -> k
    in
    function 0 -> 0 | n -> aux n 1 1

  let execute (instr : Instruction.instruction) st length =
    match instr st with
    | st, Next, cycles -> (State.adv_PC st length, cycles)
    | st, RelJump, cycles -> (State.adv_PC st length, cycles)
    ...
    let st, (instr, length) =
      ...
      | Enabled -> (
          let st, addr =
            match
              first_set_bit
              @@ IOregs.IE.get st.ie 0xFFFF
                 land IOregs.Interrupts.get st.iflag 0xFF0F
            with
            | 1 ->
                ( { st with iflag = IOregs.Interrupts.handled_VBlank st.iflag },
                  0x40 (* VBlank *) )
            | 2 ->
                ( { st with iflag = IOregs.Interrupts.handled_LCD st.iflag },
                  0x48 (* LCD *) )
            | 3 ->
                ( { st with iflag = IOregs.Interrupts.handled_timer st.iflag },
                  0x50 (* Timer *) )
            | 4 ->
                ( { st with iflag = IOregs.Interrupts.handled_serial st.iflag },
                  0x58 (* Serial *) )
            | 5 ->
                ( { st with iflag = IOregs.Interrupts.handled_joypad st.iflag },
                  0x60 (* Joypad *) )
            | _ -> (st, 0x00 (* None *))
          in
          match addr with
      ...
      | Enabled -> (
          ...
          | 1 ->
              ( {
                  st with
                  activity = Running;
                  iflag = IOregs.Interrupts.handled_VBlank st.iflag;
                },
                0x40,
                Running )
          | 2 ->
              ( {
                  st with
                  activity = Running;
                  iflag = IOregs.Interrupts.handled_LCD st.iflag;
                },
                0x48,
                Running )
          | 3 ->
              ( {
                  st with
                  activity = Running;
                  iflag = IOregs.Interrupts.handled_timer st.iflag;
                },
                0x50,
                Running )
          | 4 ->
              ( {
                  st with
                  activity = Running;
                  iflag = IOregs.Interrupts.handled_serial st.iflag;
                },
                0x58,
                Running )
          | 5 ->
              ( {
                  st with
                  activity = Running;
                  iflag = IOregs.Interrupts.handled_joypad st.iflag;
                },
                0x60,
                Running )
