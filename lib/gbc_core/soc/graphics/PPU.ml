...
module Make (State : State.S) : S with type state = State.t = struct
  ...
  let mk_pixel color palette sprite_prio prio =
    { color; palette; sprite_prio; prio }
  ...
  let dot_of_mc mcycles speed = if speed then mcycles * 2 else mcycles * 4
  ...
  let render_bgw_line (st : state) ly =
    let render_bg_line (st : state) ly tile_data_area =
      ...
      let bg_tile_map_area =
        State.GPUmem.LCD_Regs.bg_tm_area st.gpu_mem.lcd_regs
      in
      ...
      while !lx < screen_w do
        ...
        let tile_index =
          State.GPUmem.VRAM.get_tile_index st.gpu_mem.vram bg_tile_map_area y x
        in
        let tile_attr =
          State.GPUmem.VRAM.get_tile_attributes st.gpu_mem.vram bg_tile_map_area
            y x
        in
        ...
        let p1, p2 =
          State.GPUmem.VRAM.get_tile_data_row st.gpu_mem.vram tile_data_area
            tile_index row_in_tile bank
        in
        let p1 = if x_flip then ref p1 else ref (Utils.rev_u8 p1) in
        let p2 = if x_flip then ref p2 else ref (Utils.rev_u8 p2) in
        let len =
          if col_in_tile > 0 then 8 - col_in_tile
          else if screen_w - !lx < 8 then screen_w - !lx
          else 8
        in
        for i = 0 to len - 1 do
    ...
    let render_w_line (st : state) ly tile_data_area =
      ...
      if wy <= ly && ly <= wy + window_wh && wx <= screen_w then
        let window_tile_map_area =
          State.GPUmem.LCD_Regs.window_tm_area st.gpu_mem.lcd_regs
        in
        ...
        while !lx < screen_w do
          ...
          let x_in_w = !lx - wx in
          let tile_index =
            State.GPUmem.VRAM.get_tile_index st.gpu_mem.vram
              window_tile_map_area y_in_w x_in_w
          in
          let tile_attr =
            State.GPUmem.VRAM.get_tile_attributes st.gpu_mem.vram
              window_tile_map_area y_in_w x_in_w
          in
          let prio = tile_attr land 0x80 > 0 in
          let y_flip = tile_attr land 0x40 > 0 in
    ...
    in
    let tile_data_area =
      State.GPUmem.LCD_Regs.bw_base_pointer st.gpu_mem.lcd_regs
    in
  ...
  let render_line (st : state) =
    ...
    for i = 0 to screen_w - 1 do
      match
        ( bgw_buffer.(i),
          obj_buffer.(i),
          State.GPUmem.LCD_Regs.bgwindow_ep st.gpu_mem.lcd_regs )
      with
      (* No object pixel *)
    ...
    if not @@ State.GPUmem.LCD_Regs.lcd_enabled st.gpu_mem.lcd_regs then
      ...
      | GPUmode.HBlank (c, m) ->
          ...
          if new_c >= m then
            ...
            if st.gpu_mem.lcd_regs.ly < screen_h then
              let st =
                if State.GPUmem.LCD_Regs.mode2_cond st.gpu_mem.lcd_regs then
              ...
              let st =
                if State.GPUmem.LCD_Regs.mode1_cond st.gpu_mem.lcd_regs then
                  State.request_LCD st
                else st
              in
                  ...
                  gpu_mem =
                    {
                      st.gpu_mem with
                      lcd_regs =
                        State.GPUmem.LCD_Regs.reset_wlc st.gpu_mem.lcd_regs;
                    };
                }
              in
              (State.change_mode st @@ VBlank (new_c - m), false)
          else (State.update_mode st @@ HBlank (new_c, m), false)
      | GPUmode.VBlank c ->
          let new_c = c + dots in
      ...
      | GPUmode.Drawing_pixels (c, m) ->
          ...
          if new_c >= m then (
                  ...
                  gpu_mem =
                    {
                      st.gpu_mem with
                      lcd_regs =
                        State.GPUmem.LCD_Regs.inc_wlc st.gpu_mem.lcd_regs;
                    };
                }
              else st
            in
            ...
            let st =
              if State.GPUmem.LCD_Regs.mode0_cond st.gpu_mem.lcd_regs then
                State.request_LCD st
              else st
            in
            (State.change_mode st @@ HBlank (new_c, 204), false))
          else (State.update_mode st @@ Drawing_pixels (new_c, m), false)
end
