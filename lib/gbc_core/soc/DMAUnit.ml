...
module MakeOAM (State : State.S) : (S with type state = State.t) = struct
  ...
  let exec_dma (st : state) mc =
    let rec aux (st : state) (DMAState.OAM.State.Active {src;progress}) n =
      ...
      | 160, _ -> { st with dma_oam = DMAState.OAM.set_state st.dma_oam Inactive }, 0
      | progress, 0   -> { st with dma_oam = DMAState.OAM.set_state st.dma_oam @@ Active {src; progress} }, 0
      | progress, n   -> aux (State.Bus.set8 st (0xFE00 + progress) (State.Bus.get8 st (src + progress))) (DMAState.OAM.State.Active {src; progress = progress + 1}) (n - 1)
    in
...
module MakeVRAM (State : State.S) : (S with type state = State.t) = struct
  ...
  let exec_dma (st : state) _ =
    ...
    | Running, false -> st, 0
    | Running, true ->
      let rec transfer st src dst =
        function
        | 0 -> st
        | n -> transfer (State.Bus.set8 st dst (State.Bus.get8 st src)) (src + 1) (dst + 1) (n-1) in
      ...
      | false -> (* HBlank*)
        ...
        | HBlank (_, _) ->
          ...
          if length = 0 then
            ...
            let hdma1 = Utils.get_high src in let hdma2 = Utils.get_low src in
            let hdma3 = Utils.get_high dst in let hdma4 = Utils.get_low dst in
            { st with dma_vram = { hdma1; hdma2; hdma3; hdma4; hdma5; in_progress = true } }, mcyc
        | _ -> st, 0
