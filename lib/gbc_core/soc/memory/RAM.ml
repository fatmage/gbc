  ...
  (module struct
    ...
    let get mem index =
      ...
      let rec aux mem =
        ...
        | Cap (_, m), _ -> aux m
        | Node (i, l, v, r), index ->
            if index = i then v else if index < i then aux l else aux r
    ...
    let set mem index v =
      ...
      let rec aux mem =
        ...
        | Node (i, l, x, r), index ->
            if index = i then Node (i, l, v, r)
            else if index < i then Node (i, aux l, x, r)
            else Node (i, l, x, aux r)
        | _, _ -> Utils.fail_addr "no cap" index
      in
      aux mem
...
module WRAM = struct
  ...
  let initial =
    {
      b0 = Bank0.initial;
      bs = List.init 7 (fun _ -> Banks.initial);
      svbk = 0xF8;
    }
  ...
  let get m i =
    ...
    | { bs; svbk; _ }, i ->
        ...
        if bank = 0 then Banks.get (List.hd bs) i
        else Banks.get (List.nth bs (bank - 1)) i

  let set m i v =
    match (m, i) with
    | _, 0xFF70 -> { m with svbk = 0xF8 lor v }
    | { b0; _ }, i when Bank0.in_range i -> { m with b0 = Bank0.set b0 i v }
    | { bs; svbk; _ }, i ->
        let bank = if svbk land 0b111 = 0 then 1 else svbk land 0b111 in
        let aux j mem = if j = bank - 1 then Banks.set mem i v else mem in
